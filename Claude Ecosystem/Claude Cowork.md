...
graph TD
    ...
    Root --> A["Bản chất & Triết lý"]
    Root --> B["Chuyển dịch tư duy<br/>Chat → Cowork"]
    Root --> C["Tích hợp môi trường<br/>(4 trụ cột)"]
    Root --> D["Anatomy của Task chuẩn"]
    Root --> D5["Công thức Prompt chuẩn<br/>(Deliverable+Inputs+Nuances)"]
    Root --> D2["Tiêu chí chọn Task<br/>(3 Patterns)"]
    Root --> D3["Progress Panel<br/>(Bảng tiến độ)"]
    Root --> D4["Nhịp điệu Làm việc<br/>(Work Rhythm)"]
    Root --> D6["Cơ chế Câu hỏi<br/>Làm rõ (Clarifying)"]
    Root --> D7["Điều chỉnh Mid-task<br/>(Steer & Interrupt)"]
    Root --> D8["Nghiệm thu Sản phẩm<br/>(Review Deliverable)"]
    Root --> E["So sánh 3 chế độ<br/>Chat, Cowork, Code"]
    Root --> F["Cài đặt & Khởi chạy"]
    Root --> G["Chọn Thư mục &<br/>Phân quyền Đọc/Ghi"]
    Root --> H["Thêm & Quản lý<br/>Connectors"]
    Root --> H2["Cài đặt & Tùy biến<br/>Plugins"]
    Root --> H3["Tự xây dựng Plugin"]
    Root --> I["Mô hình Phân quyền<br/>(Permissions)"]
    Root --> I2["Viết Prompt An toàn<br/>(Tránh Hiểu Lầm)"]
    Root --> I3["Giám sát Real-time<br/>(Watch the Run)"]
    Root --> I4["Giới hạn Sử dụng<br/>(Khi nào KHÔNG dùng)"]
...
- [[Giám sát Real-time khi Cowork chạy (Watch the Run)]]
- [[Khi nào KHÔNG nên dùng Cowork (Giới hạn Sử dụng)]]
